...
public class Back implements CommandExecutor{
	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args){
		...
		if(!(s instanceof Player)){
			ZPort.chat.error(s, "Console cannot go back to a previous location.");
			return true;
		}
		if(args.length != 0){
		...
		if(!p.hasPermission("zp.tp.back")){
			ZPort.chat.error(s, "You don't have permission to go Back.");
			return true;
		}
		...
		if(getPreviousLocation(p) == null){
			ZPort.chat.error(s, "You don't have a previous location set.");
			return true;
		}
		...
		ZPort.chat.message(s, "You have returned to your previous location.");
