...
public class Home{
	...
	public static Home getHome(Player p, String w){
		...
		if(Backend.isSQL()){
			...
			Location l = new Location(Bukkit.getWorld(c.getString("Location.World")), c.getDouble("Location.X"), c.getDouble("Location.Y"),
					c.getDouble("Location.Z"), Float.parseFloat(c.getString("Location.Yaw")), Float.parseFloat(c.getString("Location.Pitch")));
			h = new Home(p, l);
		}
		return h;
	}
